...
async function ensureLobster(): Promise<void> {
  if (typeof document === 'undefined') return;
  if (document.fonts.check('12px Lobster')) return;
  try {
    const f = new FontFace('Lobster', 'url(https://fonts.gstatic.com/s/lobster/v28/neILzCirqoswsqX9zoKmMw.woff2)');
    await f.load();
...
export default function CreateLoofaPage() {
  ...
  const buildMainBagTemplateCanvas = async (): Promise<HTMLCanvasElement | null> => {
    ...
    if (designCanvas) {
      ...
      if (backDesign === 'duplicate') {
        ...
        ctx.rotate(Math.PI);
        // 5% base margin from BACK_BOT, shifted 20px higher in original space (+20 in rotated space)
        ctx.drawImage(designCanvas, Math.round(cx - contentW / 2), Math.round(BACK_H * 0.05) + 20, contentW, dh);
        ctx.restore();
      }
    }
  ...
  // Pocket: download the static pocket1design.jpg directly
  const downloadPocketTemplate = () => {
    const a = document.createElement('a');
    a.href = '/pocket1design.jpg';
    a.download = `loofabag-${slug || 'design'}-pocket.jpg`;
